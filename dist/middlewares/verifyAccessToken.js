...
export const verifyAccessToken = async (req, res, next) => {
  try {
    ...
    if (!authHeader) {
      return res.status(401).json({ error: "Autherization header missing" });
    }
    ...
    //check wether it is an access token
    ...
    res.status(401).json({ error: "Invalid or token is expierd" });
  }
};
...
//     //check wthere it is an access token
